...
export const weatherData = async(lat,lon) =>{
    ...
    try{
        const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?${lat}&lon=${lon}&appid=${process.env.OPEN_WEATHER_API_KEY}&units=metric`)

        const data = await response.json()

        return data?.weatherData[0]
    }catch(err){
...
export const tempretureData = async(lat,lon) =>{
